...
export function InlineSearch({ className = "", placeholder = "Search for products, brands and more", onFocusChange }: InlineSearchProps) {
  ...
  return (
    <div ref={dropRef} className={`relative w-full ${className}`}>
      {/* Search input */}
      <div className={`flex items-center w-full bg-white border-2 transition-colors rounded-sm overflow-hidden ${focused ? "border-yellow-400" : "border-gray-200"}`}>
        ...
        <button
          ...
          className="flex-shrink-0 h-10 px-5 bg-yellow-400 hover:bg-yellow-500 text-gray-900 font-semibold text-sm transition-colors"
        >
          Search
        </button>
      </div>
        ...
        <div className="absolute left-0 right-0 top-full mt-0.5 bg-white border border-gray-200 shadow-xl rounded-sm z-[200] overflow-hidden max-h-[70vh] overflow-y-auto">
          ...
          {loading && (
            <div className="flex items-center gap-2 px-4 py-3 text-sm text-gray-500">
              <span className="h-4 w-4 border-2 border-yellow-400 border-t-transparent rounded-full animate-spin" />
              Searching…
            </div>
          )}
              ...
              {hits.length > 0 && (
                <div>
                          ...
                          <div className="flex items-center gap-2 mt-0.5">
                            ...
                            {hit.ratingAvg && Number(hit.ratingAvg) > 0 && (
                              <span className="text-[11px] text-gray-500 flex items-center gap-0.5">
                                <span className="text-yellow-500">★</span>{Number(hit.ratingAvg).toFixed(1)}
                              </span>
                            )}
                          </div>
                        </div>
                  ...
                  <button
                    data-item
                    onClick={() => navigate(query.trim())}
                    className="w-full px-4 py-2.5 text-sm text-yellow-600 font-semibold text-center hover:bg-yellow-50 border-t border-gray-100"
                  >
                ...
                <div>
                  <div className="flex items-center justify-between px-4 pt-3 pb-1">
                    ...
                    <button
                      ...
                      className="text-[10px] text-yellow-600 hover:underline font-medium"
                    >
                      Clear all
                    </button>
                  </div>
                ...
                <div className="px-4 pb-2 flex flex-wrap gap-2">
                  {TRENDING.map((t) => (
                    <button
                      key={t}
                      onClick={() => navigate(t)}
                      className="flex items-center gap-1.5 px-3 py-1.5 bg-gray-50 hover:bg-yellow-50 hover:text-yellow-700 border border-gray-200 hover:border-yellow-300 rounded-full text-xs text-gray-600 font-medium transition-colors"
                    >
